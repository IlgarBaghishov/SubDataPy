...
slctd_num = 100
for slctd_num_low in [500, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000, 5500, 6000, 6500, 7000, 7500, 8000, 8500, 9000, 9500]:
    slctd = df.sort_values(by="e_cooks",ascending=False).index[:slctd_num]
    unslctd = df.sort_values(by="e_cooks",ascending=False).index[slctd_num:]
    ...
    # print("\nFitting to selected "+str(slctd_num)+" high precision configurations only")
    ...
    print("Energy RMSE if tested on the entire dataset is", np.sqrt(np.sum(entire_test_residual*energy_selector/22500)/energy_selector.sum()))
    print("Force RMSE if tested on the entire dataset is", np.sqrt(np.sum(entire_test_residual*force_selector)/force_selector.sum()))
# np.save('coefficients_imprvd_low.npy',coefficients)
...
#     detailed_results[configs_index[j][1:],1] = bw_high[configs_index[j][1:]] #forces
...
#     detailed_results[i,1] = bw_high[configs_index[j][0]]/150 #energy
