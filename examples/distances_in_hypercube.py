...
for d_fps in np.arange(0.0006,0.05,0.05):  # 0.001is267
    ...
    for i in range(trainingimages_len-2,-1,-1):  # loop through structures in all_fps list
        print("Structure",i, 20050-i-len(removed))
        unique = False
        ...
        for j in range(all_fps[i].shape[0]):  # loop through atoms in a structure
            ...
            #     continue
            summed_min_dist += np.min(np.sum(np.square(all_fps_subset - curr_struct[j]),axis=1)/max_N_fps)
        summed_min_dist = summed_min_dist/all_fps[i].shape[0]
        
        if summed_min_dist < d_fps:
            removed.append(i)
        else:
            start_row = end_row
    ...
    output_traj_directory = "../%3.6f_%d/" % (d_fps, chosen_len)
