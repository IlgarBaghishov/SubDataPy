...
bw = np.load("/vast/home/baghishov/old_entropy/qSNAP/big_files/npy_files/Be/bw_high.npy")
...
XTy = aw[hlfpnt1].T @ bw[hlfpnt1].reshape(-1,1)
...
for i in range(hlfpnt-10):
    ...
    if slctd.shape[0] in [100,200,350,500,750,1000,1500,2250,3000]:
        ...
        coefficients, *_ = lstsq(aw[hlfpnt1_high], bw[hlfpnt1_high], 1.0e-13)
        ...
        train_residual = np.square(np.dot(aw[hlfpnt1_high],coefficients) - bw[hlfpnt1_high])
    ...
    if i!=0:
        ...
        XTy -= aw[hlfpnt1_rmvd].T @ bw[hlfpnt1_rmvd].reshape(-1,1)

    coeffs = XTX_inv @ XTy
    en_residuals_squared_low = np.square(aw_slctd_en @ coeffs - bw[hlfpnt1_high_en].reshape(-1,1)).reshape(-1)
